...
- **NEW (2026-08-20, cycle 11):** a placement-exclusion CUT's basis can flip within a single week —
  ...
  **2026-09-17 update: confirmed a 4th time — Broad Interest's Facebook Feed (MA-095) re-verified
  fresh this DEEP_WEEK and is STILL the worst placement (CPP ₹1,316.54 vs blend ₹372.73, one cycle
  early re-check) — the first time a placement finding has held true two consecutive checks rather
  than flipping. A placement problem CAN persist; don't assume every re-check will find a flip.**
- **NEW (2026-08-20, cycle 11):** checkout→purchase conversion can collapse simultaneously across
  ...
  one instance resolves.** **2026-09-17 update: Broad Interest's own C2P leak (first flagged 2026-09-16
  as a recurrence) deepened further this cycle (44.2%→24.4% combined-tag, two consecutive declining
  reads) — a "recurrence" of a recovered pattern can itself keep getting worse rather than being a
  one-off blip; treat two consecutive declining C2P reads as confirmation of a real, structural leak,
  not noise.**
- **NEW (2026-08-27, cycle 12):** GA4 campaign-tag mismatches are a structural, account-wide pattern,
  ...
  **2026-09-17 update: the combined-tag rule caught the account's most severe reversal yet — Openwire's
  own-tag GA4-dir ROAS read 0.00× this cycle while its combined-tag read (0.48×, still terrible but a
  real number) showed the collapse is genuine, not a tagging artifact masking a healthy campaign. The
  UTM fix remains unshipped after 8+ days since approval — the standing combination workaround is now
  carrying real decision weight every single cycle, not just an occasional correction.**
...
- **NEW (2026-09-10, cycle 14):** a campaign can go from "efficiency decline" to "fully dark" within a
  ...
  live campaign each cycle. **2026-09-17 update: the campaign RESUMED on day 13 of what would have been
  a longer stoppage, with no logged cause for either the stoppage or the resumption — "resumed" is not
  the same as "fixed" or "explained"; treat a resumption as a fresh, thin data point requiring its own
  re-verification window (3-7 days), not a closed loop.**
...
- **NEW (2026-09-17, cycle 15):** a "first clean week above floor" (last cycle's 2026-09-16 ad-hoc
  read: raw 6.40×, matured 7.45×) can fail to hold the very next time the trailing-7d window rolls
  forward by a single day — this cycle's window (one day later) read matured 5.69×, back below floor
  and essentially flat vs two cycles ago. A single clean week is a data point, not a trend; never treat
  one clean matured read as sufficient grounds to recommend lifting an account-wide HOLD — require it
  to hold across at least two consecutive rolling windows before treating the floor as genuinely
  re-cleared.
- **NEW (2026-09-17, cycle 15):** a campaign can collapse from "severely declining" to "fully dark on
  every single day of the trailing week" in one cycle (ES Comm 2: -44.3% spend last cycle → ₹0.00 every
  day this cycle) — once a campaign crosses from partial to total non-delivery across a full 7-day
  window with a long instability history behind it (5+ prior collapses, cause never confirmed),
  continuing to re-escalate the same diagnostic ask stops being useful; the right move is a direct
  keep-or-kill decision request, not another data-only escalation.
...
- **Cycle 15 (2026-09-17) update:** no change — still closed to Meet's direct investigation; not re-escalated.
...
### 2026-09-17 — Cycle 15 (Thursday teardown, DEEP_WEEK — ISO week 38 is even): account still below the 6.0 floor (last cycle's "clean week" didn't hold); safebuds_20Apr resumed after 12 dark days; Openwire collapsed 6.09×→0.48×; ES Comm 2 fully dark all 7 days

**Initiative:** Weekly campaign-level teardown per the standing Thursday cadence. `DEEP_WEEK` computed
first (ISO week 38, even → deep-dive cycle) — pulls 7/8 (auction-quality/hook-retention, placement)
and the GA4 `addToCarts`/`checkouts` funnel metrics were correctly requested this cycle. Pulls, all
direct HTTPS inside one COMPUTE script (0 Windsor/MCP calls): Meta campaign×day 35d
(2026-08-13→2026-09-16, 147 rows, 1 page), Meta ad×day 21d (2026-08-27→2026-09-16, 298 rows, 1 page),
Meta ad-diagnostics cur7d agg (18 rows, DEEP_WEEK), Meta campaign×placement cur7d agg (69 rows,
DEEP_WEEK), GA4 account src/medium×day 14d incl. funnel metrics (401 rows, rowCount 401, PASS), GA4
campaign×src/medium cur7d (93 rows, PASS) and prior7d (91 rows, PASS) incl. funnel metrics, GA4 account
src/medium MTD (69 rows, PASS). Purchase action-type sanity check: `omni_purchase`,
`offsite_conversion.fb_pixel_purchase`, `web_in_store_purchase` all summed to an identical 832.00 on
the 35d campaign pull — no divergence. Ad-level 21d spend reconciled exactly to campaign-level 21d
spend (₹1,23,658.14 both sides). GA4 campaign sessions used raw per-row sums (cur7d 11,602, prior7d
9,823) — no dimension filter applied.

**Hypotheses tested / findings:**

1. **Does the account clear the 6.0 floor and marginal-ROAS gate this week?**
   **Result: NO — still below floor; last cycle's brief "clean week" did not hold.** Raw cur7d
   (10–16 Sep): spend ₹35,851.50, GA4 Meta-paid revenue ₹1,74,512.35 → TRUE ROAS **4.87×**. 16 Sep
   (newest day) carried a 47.4% `(not set)` share vs 0% every other day this window — matured 6d
   (10–15 Sep): TRUE ROAS **5.69×** (prior matured, cycle 14: 5.65×) — still below floor, essentially
   flat. Marginal ROAS (matured-vs-matured): Δspend/day +₹211.52 (+4.5%), Δrevenue/day +₹1,375.53
   (+5.2%) → 6.50×/incremental rupee — technically clears the gate but reflects thin organic drift,
   not a deliberate scale step; not a basis to add budget. MTD (1–16 Sep): spend ₹84,356.57, revenue
   ₹5,21,723.15, TRUE ROAS 6.18× — above floor, propped up by early September. Platform overclaim
   94.7% cur7d — within the normal band, no attribution break.
2. **Did last cycle's "first clean week above floor" (raw 6.40×/matured 7.45×, logged 2026-09-16) hold?**
   **Result: NO.** Once the trailing-7d window rolled forward one more day, matured TRUE ROAS fell back
   to 5.69× — below floor again, and barely different from cycle 14's matured 5.65× two cycles ago. The
   "clean week" was a single data point produced by that specific window, not evidence of a sustained
   recovery. HOLD (MA-083) stays in force, no exceptions.
3. **safebuds_20Apr — still fully dark, or has anything changed?**
   **Result: RESUMED after ~12 consecutive dark days.** Spend ₹0.00/absent 09-04 through 09-15, then
   ₹703.64 on 09-16 (1 day only). Cause of the original stoppage still unconfirmed by Meet/Ads Manager.
   Too thin (1 day, 0 platform purchases, GA4-dir ROAS 4.26×) to treat as resolved — FIX drafted
   (**MA-098**) requesting a 3–7 day re-verification window before re-executing MA-080/081.
4. **ES Comm 2 — still unstable?**
   **Result: total collapse — ₹0.00 spend every single day of the cur7d window** (10–16 Sep), the 6th
   consecutive cycle of instability and the first with zero spend on every day, not just a partial
   decline. Escalated as a formal keep/kill decision request (**MA-100**) rather than a 6th
   data-only re-escalation of MA-085's unconfirmed creative-retirement correlation.
5. **Openwire — has the UTM-tag mismatch (MA-076/077) resolved, and where does it read now?**
   **Result: severe reversal, the account's worst this cycle.** Combined-tag GA4-dir ROAS collapsed
   from 6.09× (two cycles ago) to **0.48×** this cycle while spend fell -38.6% (₹2,667.02→₹1,637.51).
   This is measured on the combined tag, so it is not explained by the standing UTM mismatch — something
   changed in the campaign's own delivery, audience, or creative mix. Escalated for diagnosis
   (**MA-099**).
6. **ES Lite — any change from its closed status (MA-056)?**
   **Result: relaunched with 1 new ad, 1 day of spend.** "Arjun Influencer Reel_ES Lite_16 Sep" shows
   CPP ₹192.49, platform ROAS 9.85×, GA4-dir ROAS 5.25× on a single day — encouraging but far too thin
   to judge. Read-plan set (**MA-101**, CPP≤₹400/GA4-dir ROAS≥8× by D+7 2026-09-23).
7. **Broad Interest — DEEP_WEEK diagnostics: placement, funnel, hook retention, auction quality.**
   **Result: all four re-confirmed, mostly re-verifying yesterday's (2026-09-16) ad-hoc findings, with
   the checkout leak worsening.** Placement: Facebook Feed re-verified still the worst placement (CPP
   ₹1,316.54 vs blend ₹372.73, +253%) — recommend executing MA-095 now. Funnel: C2P deepened further,
   combined-tag 44.2%→24.4% (own-tag 32.8%→23.7%) — escalated (**MA-103**), the leak is worsening not
   stabilizing. Hook retention: hero ad's p75/p25 essentially unchanged at 24.2%, still weak — MA-097's
   re-edit test remains the right lever. Auction quality: clean, no `BELOW_AVERAGE` rankings on any
   real-spend ad this week.
8. **New zero-purchase ads found this cycle?**
   **Result: one new sibling ad found.** "Before After Comparision | Safebuds Static | 25 Aug 26"
   (non-Copy variant, distinct from the already-flagged "–Copy" sibling in MA-082), ₹183.20/21d, 0
   purchases, on safebuds_20Apr — CUT drafted (**MA-102**). The standing MA-090/082 zero-purchase CUTs
   remain unexecuted, re-confirmed with fresh numbers.
9. **Format engine — any change?** **Result: append-only, no rotation change.** CF-31/37/51/54/55/56/59
   remain the standing register, still not delivering per MA-059 (standing Meet agenda item, not
   re-drafted). Appended net-new format **CF-60 "Employee Product Review (Not a Paid Actor)"**
   (**MA-104**), motivated by Broad Interest's hero-ad concentration (69.6% of spend) and weak
   retention (24.2%) needing a visually distinct, non-reviewer-POV challenger.
...
Broad Interest's C2P leak deepening, not stabilizing); MA-104 (TEST, append CF-60); D+7/re-verification
status updates on MA-048 (Broad Interest budget step, still HOLD), MA-077 (Broad Interest UTM fix,
widest own/combined gap yet, still unshipped), MA-078 (Pulse re-confirmed, spend tripled organically),
MA-083 (HOLD re-confirmed, last cycle's clean week did not hold), MA-090/082 (zero-purchase cuts still
unexecuted), MA-095 (Broad Interest Facebook Feed placement re-verified, still valid), MA-096 (hero-ad
concentration moderated slightly to 69.6%, still elevated), MA-097 (hero-ad retention re-verified, still
weak).

**Learning carried forward:**
- **A single clean week above the ROAS floor is a data point, not a trend.** Last cycle's matured 7.45×
  read (the account's first clean week in 4+ cycles) did not survive the trailing-7d window rolling
  forward by one day — this cycle's matured read is back to 5.69×. Require at least two consecutive
  rolling-window clean reads before treating any account floor-clearing as a genuine recovery worth
  recommending a HOLD-lift on.
- **"Resumed" and "resolved" are different signals.** safebuds_20Apr's 12-day stoppage ended with a
  single day of thin spend and no confirmed cause — treating this as "fixed" and immediately
  re-executing the pending pause/placement-exclusion drafts would risk acting on a stale basis if the
  campaign goes dark again. A resumption needs its own re-verification window before any pending action
  tied to the dark period is treated as current.
- **A campaign's instability history can cross a threshold where continued re-escalation stops being
  useful.** ES Comm 2's move from "partial collapse" to "zero spend every day of the trailing week,"
  its 6th consecutive unstable cycle, converted this from a diagnostic-escalation problem into a
  keep-or-kill decision problem — recognize when repeating the same ask a 6th time is unlikely to
  change the outcome and reframe the ask accordingly.
- **The standing UTM combined-tag rule (MA-037/076/077) is now carrying real decision weight every
  cycle, not just occasional correction** — this cycle it distinguished a genuine severe collapse
  (Openwire, 0.48× combined-tag) from what would have looked like total zero (0.00× own-tag). The
  longer the UTM fix stays unshipped, the more this manual-combination workaround becomes a permanent,
  error-prone part of the weekly process rather than a temporary patch.
...
>
> **Cycle 15 (2026-09-17) note:** CF-31/37/51/54/55/56/59 confirmed still not delivering — standing
> Meet agenda item (MA-059), not re-drafted weekly. Appended net-new format **CF-60 "Employee Product
> Review (Not a Paid Actor)"** (MA-104) — an authenticity/social-proof lever distinct from every
> existing register entry, motivated by Broad Interest's hero-ad concentration (69.6% of spend) and
> weak mid-video retention (24.2%) needing a visually and tonally distinct challenger.
>
...
| # | Format | Angle / psych lever | Status | Best ROAS seen | Verdict / notes | Linked EXP/MA |
|---|---|---|---|---|---|---|
| CF-31 | Expert / audiologist explainer | Authority — ENT/audiologist on "the science of open-ear" (strongest fit for NG's hearing-health moat) | **IN TEST (since cycle 6) — NOT DELIVERING, 11 straight cycles (0 impressions)** | — | Blocked on Meet's compliance call re: presenter-credential framing, unresolved 8+ weeks. Standing Meet agenda item (MA-059), not re-drafted weekly. | MA-034, MA-046, MA-052, MA-059 |
...
| CF-37 | Fear-of-loss health hook | Loss aversion — "what sealed earbuds are doing to your hearing" (WHO-stat, on-brand) | **IN TEST (since cycle 6) — NOT DELIVERING, 11 straight cycles (0 impressions); host adset ES Comm 2 is now fully dark all 7 days (MA-100)** | — | No stated production blocker, host adset now fully dark. Standing Meet agenda item (MA-059). | MA-035, MA-046, MA-052, MA-059 |
...
| CF-51 | Silent Sensory Demo (cycle 6) | Demonstration route, no voiceover — captions + ambient sound close-up of the open-ear driver/mechanism; answers the DNB-guidebook risk that NG's two winners (Gadgetbhai, Techopedia) are both reviewer-POV and may look visually too similar to each other | **IN TEST (rotated in cycle 7, 2026-08-06) — NOT DELIVERING, 11 straight cycles (0 impressions, brief still pending production); host adset (safebuds_20Apr) resumed 09-16 after 12 dark days (MA-098)** | — | Was rotated in specifically because CF-31/CF-37 weren't delivering; it also isn't delivering, and its host only just resumed. See MA-059/MA-098. | MA-045, MA-052, MA-059 |
...
| CF-56 | Checkout-Trust Overlay (cycle 11) | Trust/guarantee lever — a specific proof card (COD availability, return-window length, warranty terms) shown immediately before the CTA in the final 3–5 seconds, distinct from CF-38's broader/earlier-placed risk-reversal messaging by targeting the exact funnel stage this cycle's data shows leaking | UNTESTED | — | Appended 2026-08-20 (cycle 11), motivated directly by that cycle's funnel diagnostic finding a checkout→purchase collapse shared across Broad Interest and Openwire (MA-071); that original collapse has since RECOVERED (cycle 13) but recurred and deepened in isolation on Broad Interest (MA-094/103) — the format idea is now the single most overdue test on the account given the deepening leak. Recommend it jump the rotation queue ahead of the older, still-blocked CF-31/37/51. | MA-073 |
...
| CF-59 | Silent Subtitle Testimonial (cycle 14) | A muted-autoplay-safe lever — bold on-screen captioned testimonial (real customer language, no voiceover reliance, no ASMR/sound-texture dependence), distinct from CF-51 (silent product demo, not a testimonial) and CF-57/58 (sound-reliant ASMR-unboxing) | UNTESTED | — | Appended 2026-09-10 (cycle 14), motivated by Broad Interest now carrying 79% of the account's live spend — the only campaign with enough real volume to safely host a new challenger test this cycle. Next in line once a slot frees. | MA-093 |
| CF-60 | Employee Product Review (Not a Paid Actor) (NEW, cycle 15) | Authenticity/social-proof lever — a first-person, unscripted employee testimonial ("I work here, here's my honest take"), distinct from CF-34 (rapid review-wall montage of many customers) and CF-49 (founder reading a 1-star review) | UNTESTED | — | Appended 2026-09-17 (cycle 15), motivated by Broad Interest's hero-ad concentration (69.6% of spend, MA-096) and weak mid-video retention (24.2%, MA-097) needing a visually and tonally distinct challenger rather than another reviewer-POV clone. Next in line once a slot frees. | MA-104 |
...
| SH-MA-10 | MOFU_PDP & Video Thruplay _24 May 26 campaign, once fully paused, frees ₹3,492.64/week with zero measurable loss to any converting funnel stage (it has driven 0 purchases 2 straight weeks) — the freed spend funds SH-MA-7 reallocation + the CF-31/CF-37 format tests without needing net-new budget. | Pause the full campaign (MA-031); confirm at D+7 spend→₹0 and no revenue/CVR impact elsewhere. | **CONFIRMED HELD** — the campaign no longer appears at all in the 35-day campaign pull, re-confirmed again this cycle (cycle 15). | MA-031 |
| SH-MA-11 | CF-31 "Expert/audiologist explainer" challenger ad inside the safebuds_20Apr 26 LAL adset clears the review-format control (CPP ≤ ₹380, ROAS ≥ 8×, freq < 2.5×) within a 7-day read at ~₹5,000 spend. | ₹5,000 test, 7-day read, funded by MOFU-cut reallocation (MA-031). | **STALLED 11 cycles running — blocked on Meet's compliance call, 0 impressions delivered. Standing Meet agenda item (MA-059).** | MA-034, MA-046, MA-052, MA-059 |
| SH-MA-12 | CF-37 "Fear-of-loss health hook" challenger ad inside the ES Comm 2 adset clears the review-format control (CPP ≤ ₹450, ROAS ≥ 8×, freq < 2.5×) within a 7-day read at ~₹5,000 spend. | ₹5,000 test, 7-day read, funded by MOFU-cut reallocation (MA-031). | **STALLED — 0 impressions delivered; host adset (ES Comm 2) is now fully dark all 7 days, a keep/kill decision pending (MA-100).** | MA-035, MA-046, MA-052, MA-059 |
| SH-MA-13 | CF-51 "Silent Sensory Demo" challenger ad inside the safebuds_20Apr 26 hero adset clears the review-format control (CPP ≤ ₹380, ROAS ≥ 8×, freq < 2.5×) within a 7-day read at ~₹5,000 spend. | ₹5,000 test, 7-day read, funded by MA-042/043 reallocation (~₹1,036/14d) — no net-new spend. | **STALLED — brief still not produced, 11 cycles after being rotated in; host adset only just resumed after 12 dark days (MA-098).** | MA-045, MA-052, MA-059 |
| SH-MA-14 | Openwire (Type-C wired) campaign clears CPP ≤ ₹350 and platform ROAS ≥ 8× by 2026-08-12 (7-day read from its 2026-08-05 launch), consistent with the 2026-08-05 daily brief's 5.17% PDP CVR signal (vs SafeBuds' 0.75%). | No change — campaign already live at its current small spend; this is a read-plan/threshold only. | **Cycle 15 update: combined-tag GA4-dir ROAS COLLAPSED to 0.48× (from 6.09× two cycles ago) while spend fell -38.6% — the account's most severe single-campaign reversal this cycle, escalated for diagnosis (MA-099).** | MA-047, MA-062, MA-076, MA-099 |
| SH-MA-15 | "Broad Interest Based-24 Jul 26" campaign's +20% budget step (MA-048) holds GA4-directional ROAS ≥8× and platform CPP ≤₹380 within a 7-day read, confirming the organic-growth signal was real and not a one-week artifact. | +20% budget step, ~₹1,515/day → ~₹1,818/day, 7-day read. | **Cycle 15 update: combined-tag GA4-dir ROAS fell further to 6.68× (prior 9.32×) — still above floor but declining, consistent with a worsening checkout leak (MA-103) and hero-ad concentration (MA-096). Still gated behind MA-083's account-wide HOLD.** | MA-048 |
| SH-MA-16 | Openwire_5 Aug 26's first-ever +20% budget step (MA-062, ~₹1,057.58/day → ~₹1,269/day) holds CPP ≤₹350, platform ROAS ≥8×, and GA4-directional ROAS ≥6× within a 7-day read, confirming the SKU's organic-growth signal survives an actual budget increase. | +20% budget step, 7-day read. | **DEAD, cycle 15 — Openwire has since collapsed to 0.48× combined-tag GA4-dir ROAS (MA-099); this budget step stays dead regardless of the account-wide HOLD.** | MA-062, MA-076, MA-099 |
| SH-MA-17 | Re-editing the 8–20s body of "Tech in Gujarati Reel_Openwire Hindi Dub_5 Aug" (leaving the 0–2s hook untouched) raises its video_p75/p25 watch-through ratio from 24.0% to ≥40% while holding CPP ≤₹120 and platform ROAS ≥8× within a 7-day read — isolating body-retention as a fixable, separate problem from the ad's already-strong hook. | ~₹3,000 test spend, 7-day read, funded by CUT-list reallocation (MA-063/064), no net-new budget. | **STILL NOT PRODUCED — Openwire itself has since collapsed (MA-099), making this re-edit test secondary to diagnosing that collapse first.** | MA-066 |
| SH-MA-18 | A checkout-flow technical/UX fix (once diagnosed per MA-071) recovers checkout→purchase conversion in Broad Interest and Openwire back toward their pre-collapse levels within 7 days of the fix landing, confirming a shared technical cause rather than two coincidental campaign-level declines. | No spend — diagnostic + technical fix, re-read via GA4 funnel metrics at D+7. | **✅ CONFIRMED (cycle 13), but the underlying failure mode has since recurred and deepened in isolation on Broad Interest (MA-094, then MA-103) — the technical/payment audit this hypothesis called for has still never happened.** | MA-071, MA-094, MA-103 |
| SH-MA-19 | New campaign "Pulse Fitness Band \| 24 Aug 26" clears CPP ≤₹450 and GA4-directional ROAS ≥8× by D+7 (2026-08-31), consistent with its early read (CPP ₹408.53, GA4-dir ROAS 8.64× on ₹1,225.59 spend, CVR 0.82% — own-tag GA4 match confirmed direct and reliable). | No change — campaign already live at its current small spend; this is a read-plan/threshold only. | **✅ VALIDATED, re-confirmed cycle 15 — spend nearly TRIPLED organically (+192%, ₹6,841.58 cur7d) while ROAS held/improved (platform 13.30×, GA4-dir 13.34×) — still the account's clearest scale candidate, still gated behind the account-wide TRUE ROAS floor clearing (MA-083).** | MA-078 |
| SH-MA-20 | A checkout-flow/offer audit specific to safebuds_20Apr's PDP and creative mix (motivated by MA-084) recovers its checkout→purchase rate from 15.4% back toward its pre-collapse baseline (32.0%) within 7 days of MA-080/081 (hero-ad pause + placement exclusion) executing, confirming the leak was creative/placement-driven rather than a separate checkout-technical issue. | No spend — diagnostic + creative/placement fix (MA-080/081), re-read via GA4 funnel metrics at the next DEEP_WEEK (2026-09-17). | **Cycle 15 update: campaign resumed 09-16 (MA-098) but only 1 thin day of data — re-diagnose once a fuller week of resumed delivery exists.** | MA-084, MA-098 |
| SH-MA-21 | Excluding safebuds_20Apr's Facebook Reels placement (CPP ₹1,138.98 vs blend ₹566.49) improves the campaign's blended CPP toward ₹566 or better within a 7-day read, recovering roughly ₹4,008/week in excess spend. | No spend change — placement exclusion only. | **Cycle 15 update: campaign resumed 09-16 but at trivial 1-day spend — re-verify the placement mix once a fuller resumed week exists before executing.** | MA-081 |
| SH-MA-22 | Pausing/deprioritizing "Techopedia Safebuds Reel_17May 26" (CPP ₹1,026.02, ROAS 2.62×) and reallocating priority to "Nobody Tech Safebuds Reel_30Jun26" + the two new SafeBuds ads lifts safebuds_20Apr's blended CPP/GA4-directional ROAS back toward or above the 6.0 floor within a 7-day read, with no net-new spend. | Pause/deprioritize one ad, reallocate priority within the existing adset — no budget change. | **Cycle 15 update: campaign resumed 09-16 with a different ad live; this specific ad still does not reappear in the pull — execute the pause formally once/if it does.** | MA-080, MA-098 |
| SH-MA-23 | Confirming and resolving the cause of safebuds_20Apr's 6-consecutive-day full stoppage (MA-089) restores the campaign to real delivery within 3 days of Meet's investigation, at which point MA-080/081's already-approved fixes can execute meaningfully. | No spend — diagnostic escalation only; re-read at D+3 (2026-09-13). | **✅ PARTIALLY CONFIRMED — the campaign resumed spending 2026-09-16 after ~12 total dark days (longer than the D+3 window), with no cause ever confirmed by Meet. Treat as a resumption, not a resolution — see the fresh re-verification hypothesis SH-MA-25/MA-098.** | MA-089, MA-098 |
| SH-MA-24 (NEW, cycle 15) | ES Lite's relaunch ad ("Arjun Influencer Reel_ES Lite_16 Sep") clears CPP ≤₹400 and GA4-dir ROAS ≥8× by D+7 (2026-09-23), consistent with its first-day read (CPP ₹192.49, platform ROAS 9.85×, GA4-dir ROAS 5.25× on 31 sessions). | No change — campaign already live at its current small spend; read-plan/threshold only. | **NEW — drafted 2026-09-17 (cycle 15), see MA-101.** | MA-101 |
| SH-MA-25 (NEW, cycle 15) | safebuds_20Apr's resumed delivery (after ~12 dark days, MA-089/MA-098) holds for at least 3 consecutive days without going dark again, at which point MA-080/081's approved fixes should be re-verified against current data and executed. | No spend — diagnostic re-verification only; re-read at D+3 (2026-09-20). | **NEW — drafted 2026-09-17 (cycle 15), see MA-098.** | MA-098 |
