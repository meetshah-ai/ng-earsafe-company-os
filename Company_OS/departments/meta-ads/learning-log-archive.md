...
> Full cycle-by-cycle history through 2026-07-13, moved out of `learning-log.md` on 2026-08-13 to
> control the token cost of the weekly managed-agent run (that file is re-read in full every cycle).
> Nothing here is deleted — every learning that still matters was already folded into
> `learning-log.md`'s compact sections (CONFIRMED PATTERNS / REJECTED-DEAD-ENDS / CREATIVE-FORMAT
> TEST ENGINE / SCALE HYPOTHESIS BACKLOG) before this cut. **The managed agent never reads this
> file.** This is historical reference for a human only.
...
**Cycle 9 (2026-08-13) follow-up:** ES Lite is now fully dark for a 10th+ consecutive day (last real spend 2026-08-01) — still no logged cause 3 straight cycles running. See MA-056.
