...
| **Google Ads** | **TRUE ROAS ≥ 6.0 floor** (= Google Ads spend ÷ **GA4 `google/cpc` revenue** — never platform-claimed, which overstates by ~49%); operating band 9–11×; **scale only when the last increment's marginal ROAS ≥ 6.0**, measured ≥7d after it landed; **budget steps capped at ±20%/week** (a bigger step resets Smart Bidding into a 1–2 week learning phase). Set by CEO 2026-07-13. | `departments/google-ads/learning-log.md` |
...
| **Google Ads** | **Decode the week's paid-search performance and recommend — hold ROAS, and scale only where the margin justifies it.** Google Ads is intent *capture*: it harvests demand Meta and organic create. The account is **Lost-IS-Rank limited, not budget limited**, so scale comes from measurement → AI Max → Quality Score → Shopping feed → **budget last**. Every recommendation is a draft; the agent holds no write credential. | **TRUE ROAS ≥ 6.0 floor** (Google Ads spend ÷ GA4 `google/cpc` revenue), operating band **9–11×**, scale only on **marginal ROAS ≥ 6.0** in **≤20% steps**. **Hard deadline: the conversion-tracking fix + tROAS re-baseline must land before 17 Aug 2026**, when Google begins pulling budget-limited tROAS campaigns *down* toward their target. | **Twice weekly (Mon + Thu, 07:30 IST).** Monday reads the completed week; Thursday checks what Monday recommended. Never read a window younger than 7 days — the attribution window is 7 days. |
...
| Google Ads (intent capture; TRUE ROAS ≥ 6.0 floor, scale on marginal ROAS — CEO 2026-07-13) | `google-ads` | `Company_OS/agents/google-ads.agent.yaml` (**Managed Agent**, Mon + Thu 07:30 IST — no Claude Code subagent by design) | 🟢 Live |
